...
public class SortingMoveBehavior extends Behavior<EntityMaid> {
    ...
    protected boolean checkExtraStartConditions(@NotNull ServerLevel worldIn, @NotNull EntityMaid owner) {
        return MemoryUtil.getCurrentlyWorking(owner) == ScheduleBehavior.Schedule.SORTING && MemoryUtil.getSorting(owner).hasAny();
    }

    @Override
    protected void start(ServerLevel level, EntityMaid maid, long p_22542_) {
        SortingMemory sorting = MemoryUtil.getSorting(maid);
        ...
        if (goal == null) {
            sorting.addFailCount();
            if (sorting.getFailCount() > 20) {
                sorting.removeFirst();
                sorting.clearTarget();
