...
public class LogisticsOutputBehavior extends Behavior<EntityMaid> {
    ...
    protected void tick(ServerLevel p_22551_, EntityMaid maid, long p_22553_) {
        ...
        if (currentSlot < availableInv.size()) {
            ...
            if (!stack.isEmpty())
                if (context instanceof IStorageInsertableContext isic) {
                    ...
                    int insertedCount = toInsert - notInserted.getCount();
                    layer.memorizeItem(stack.copyWithCount(insertedCount), Integer.MAX_VALUE);
                    if (insertedCount > 0) {
                        try (net.neoforged.neoforge.transfer.transaction.Transaction tx = net.neoforged.neoforge.transfer.transaction.Transaction.open(null)) {
                            availableInv.extract(currentSlot, net.neoforged.neoforge.transfer.item.ItemResource.of(stack), insertedCount, tx);
                            tx.commit();
                        }
                    }
                }
        ...
        if (context != null)
            if (currentSlot >= availableInv.size() || context.isDone()) {
                ...
                if (!InvUtil.isEmpty(availableInv) && !MemoryUtil.getLogistics(maid).hasMultipleGuide(maid)) {
                    if (currentSlot >= availableInv.size()) currentSlot = 0;
