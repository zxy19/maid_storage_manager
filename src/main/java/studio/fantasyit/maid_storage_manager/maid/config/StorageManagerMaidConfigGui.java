...
public class StorageManagerMaidConfigGui extends MaidTaskConfigGui<StorageManagerMaidConfigGui.Container> {
    ...
    protected void initAdditionData() {
        this.currentMAData = StorageManagerConfigData.get(this.maid);
    }
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        graphics.pose().translate((float) startLeft, startTop - scrollOffset);
        renderScrollList(graphics, mouseX, mouseY, startLeft, (int) (startTop - scrollOffset));
        graphics.pose().popMatrix();
        graphics.pose().pushMatrix();
        graphics.pose().translate((float) startLeft, (float) startTop);
    ...
    private static final ImageAsset ROW_BG_HOVER = new ImageAsset(Identifier.fromNamespaceAndPath(MaidStorageManager.MODID, "textures/gui/maid_work_setting.png"), 1, 136, 154, 13);
    private static final ImageAsset ROW_BG = new ImageAsset(Identifier.fromNamespaceAndPath(MaidStorageManager.MODID, "textures/gui/maid_work_setting.png"), 1, 123, 154, 13);
    private static final ImageAsset BAR = ImageAsset.from4Point(Identifier.fromNamespaceAndPath(MaidStorageManager.MODID, "textures/gui/maid_work_setting.png"), 0, 0, 8, 111);
    private static final ImageAsset BAR_BLOCK = ImageAsset.from4Point(Identifier.fromNamespaceAndPath(MaidStorageManager.MODID, "textures/gui/maid_work_setting.png"), 0, 112, 6, 120);
    private static final ImageAsset BAR_BLOCK_ACTIVE = ImageAsset.from4Point(Identifier.fromNamespaceAndPath(MaidStorageManager.MODID, "textures/gui/maid_work_setting.png"), 7, 112, 12, 120);

    int lastScrolling = -1;
    long startMills = -1;

    private void renderScrollList(GuiGraphicsExtractor graphics, int x, int y, int baseX, int baseY) {
    ...
    public void drawCenteredStringWithoutShadow(GuiGraphicsExtractor graphics, Font pFont, Component pText, int pX, int pY, int pColor, int maxWidth) {
        ...
        graphics.text(pFont, pText, (int) ((pX - (float) alignWidth / 2) / scale), (int) ((pY - 3 + (14 - 8 * scale) / 2) / scale), pColor);
        graphics.pose().popMatrix();
    }

    public void drawScrollingStringWithoutShadow(GuiGraphicsExtractor graphics, Font pFont, Component pText, int pX, int pY, int pColor, int maxWidth, int baseX, int baseY, boolean enableScroll) {
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        double p_97752_ = event.x();
        double p_97753_ = event.y();
