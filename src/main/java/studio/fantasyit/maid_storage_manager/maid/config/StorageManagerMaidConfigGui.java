...
public class StorageManagerMaidConfigGui extends MaidTaskConfigGui<StorageManagerMaidConfigGui.Container> {
    ...
    protected void initAdditionWidgets() {
        ...
        this.options.add(new OptionRow(
                ...
                Component.literal(String.valueOf(this.currentMAData.itemTypeLimit())),
                button -> {
                    this.currentMAData.itemTypeLimit(switch (this.currentMAData.itemTypeLimit()) {
                        case 0, -1 -> -1;
                        default -> this.currentMAData.itemTypeLimit() / 2;
                    });
                    button.setValue(Component.literal(String.valueOf(this.currentMAData.itemTypeLimit())));
    ...
    private void renderScrollList(GuiGraphics graphics, int x, int y) {
        ...
        for (int i = 0; i < options.size(); i++) {
            ...
            graphics.drawString(font, options.get(i).label(), 5, i * 13 + 3, 0x444444, false);
            drawCenteredStringWithoutShadow(graphics, font, options.get(i).value().getValue(), 132, i * 13 + 3, ChatFormatting.GREEN.getColor());
        }
    }
