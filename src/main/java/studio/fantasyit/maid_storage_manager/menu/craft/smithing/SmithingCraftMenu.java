...
public class SmithingCraftMenu extends AbstractCraftMenu<SmithingCraftMenu> {
        ...
        switch (type) {
            case SET_ALL_INPUT -> {
                ...
                for (int i = 0; i < list.size(); i++) {
                    CompoundTag tag = list.getCompound(i).get();
    ...
    public void recalculateRecipe() {
        ...
        recipe.ifPresentOrElse(smithingRecipe -> {
            SmithingRecipeInput recipeInput = new SmithingRecipeInput(items.get(0), items.get(1), items.get(2));
            ItemStack resultItem = smithingRecipe.value().assemble(recipeInput);
