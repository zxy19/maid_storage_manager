...
public class TaczCraftMenu extends AbstractCraftMenu<TaczCraftMenu> {
    ...
    public TaczCraftMenu(int p_38852_, Player player) {
        ...
        List<RecipeHolder<GunSmithTableRecipe>> allRecipesFor = TaczRecipe.getAllRecipesForBlockId(player.level(), blockId);
        RegistryAccess registryAccess = player.level().registryAccess();
        allRecipesFor.forEach(recipe -> {
            taczRecipes.add(new Pair<>(recipe.value().getResultItem(registryAccess), recipe.id().toString()));
        });
    }
    ...
    public void recalculateRecipe() {
        ...
        List<RecipeHolder<GunSmithTableRecipe>> allRecipesFor = level.getRecipeManager().getAllRecipesFor(ModRecipe.GUN_SMITH_TABLE_CRAFTING.get());
        GunSmithTableRecipe recipe = allRecipesFor.stream().filter(r -> r.id().toString().equals(recipeId)).findFirst().map(RecipeHolder::value).orElse(null);
