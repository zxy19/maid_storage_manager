...
public class FurnaceCraftMenu extends AbstractCraftMenu<FurnaceCraftMenu> {
    ...
    public void handleGuiPacket(CraftGuideGuiPacket.Type type, int key, int value, @Nullable CompoundTag data) {
        switch (type) {
            case SET_ALL_INPUT -> {
                ListTag list = data.getList("inputs").get();
                for (int i = 0; i < list.size(); i++) {
                    ItemStack stack = ItemStackUtil.parseStack(registryAccess(), list.getCompound(i).get());
    ...
    public void recalculateRecipe() {
        if (stepDataContainer.getItem(1).getBurnTime(RecipeType.SMELTING, player.level().fuelValues()) == 0) {
        ...
        recipe.ifPresentOrElse(craftingRecipe -> {
            ItemStack resultItem = craftingRecipe.value().assemble(new SingleRecipeInput(ItemStack.EMPTY));
