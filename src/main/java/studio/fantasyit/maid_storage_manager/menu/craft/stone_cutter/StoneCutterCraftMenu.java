...
public class StoneCutterCraftMenu extends AbstractCraftMenu<StoneCutterCraftMenu> {
        ...
        switch (type) {
            ...
            case SET_ALL_INPUT -> {
                if (data != null) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        stepDataContainer.setItem(i, ItemStackUtil.parseStack(registryAccess(), list.getCompound(i).get()));
                    }
                    recalculateRecipe();
                    save();
                }
    ...
    public void recalculateRecipe() {
        ...
        if (!stepDataContainer.getItem(0).isEmpty()) {
            ...
            if (!recipe.isEmpty()) {
                availableItems = recipe.stream().map(re -> re.value().assemble(new SingleRecipeInput(ItemStack.EMPTY))).toList();
