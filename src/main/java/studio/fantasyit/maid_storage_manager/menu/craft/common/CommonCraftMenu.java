...
public class CommonCraftMenu extends AbstractContainerMenu implements ISaveFilter, ICraftGuiPacketReceiver {
    ...
    public void handleGuiPacket(CraftGuideGuiPacket.Type type, int key, int value, @Nullable CompoundTag data) {
        switch (type) {
            ...
            case SET_MODE -> {
                if (data != null) {
                    Identifier action = Identifier.fromNamespaceAndPath(data.getString("ns").orElse(""), data.getString("id").orElse(""));
            ...
            case OPTION -> {
                if (data != null)
                    currentEditingItems.setOption(key, value, data.getString("value").orElse(""));
                save();
            }
            case SET_ITEM -> {
                if (data != null) {
            ...
            case SET_ALL_INPUT -> {
                if (player instanceof ServerPlayer sp) {
                    ListTag inputTag = data.getList("inputs").get();
                    ListTag outputTag = data.getList("outputs").get();
                    int inputId = 0;
                    int outputId = 0;
                    ...
                    for (int id = 0; id < craftGuideData.steps.size(); id++) {
                        ...
                        for (int i = 0; i < step.step.actionType.inputCount(); i++) {
                            if (inputId < inputTag.size()) {
                                ItemStack tmp = ItemStackUtil.parseStack(player.registryAccess(), inputTag.getCompound(inputId).get());
                        ...
                        for (int i = 0; i < step.step.actionType.outputCount(); i++) {
                            if (outputId < outputTag.size()) {
                                ...
                                ItemStack tmp = ItemStackUtil.parseStack(player.registryAccess(), outputTag.getCompound(outputId).get());
            ...
            case GENERATOR -> {
                if (generatedRecipes.craftGuideSuppliers.size() > value) {
                    ...
                    ListTag inputs = data.getList("inputs").get();
                    List<ItemStack> inputsList = new ArrayList<>();
                    for (int i = 0; i < inputs.size(); i++) {
                        inputsList.add(ItemStackUtil.parseStack(player.registryAccess(), inputs.getCompound(i).get()).copyWithCount(counts.get(i)));
                    }
