...
public class CommunicateMarkMenu extends AbstractContainerMenu implements ISaveFilter {
    ...
    public void handlePacket(CommunicateMarkGuiPacket p) {
        switch (p.type) {
            ...
            case SET_ITEMS -> {
                ListTag list = p.data.getListOrEmpty("list");
                for (int i = 0; i < Math.min(list.size(), item.requires.size()); i++) {
                    ItemStack stack = ItemStackUtil.parseStack(player.registryAccess(), list.getCompound(i).get());
            ...
            case USE_ID -> {
                Identifier id = Identifier.tryParse(CommunicateMarkGuiPacket.getStringFrom(p.data));
