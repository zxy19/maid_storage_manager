...
public class CommunicateMethodSelector extends AbstractWidget {
    ...
    static final Identifier background = Identifier.fromNamespaceAndPath(MaidStorageManager.MODID, "textures/gui/communicate_terminal.png");
    ...
    List<SelectButtonWidget<Identifier>> buttons = new ArrayList<>();
    List<CommunicateRollingTextWidget> texts = new ArrayList<>();
    final List<Identifier> methods;
    int currentId = 0;
    int mOffset = 0;
    boolean visible = false;
    Consumer<Identifier> callback = null;
    ...
    public Function<@Nullable Identifier, SelectButtonWidget.Option<Identifier>> getIdForButtonI(int i) {
        return ca -> {
            if (ca == null) {
    ...
    public void setCallback(Consumer<Identifier> callback) {
    ...
    @Override
    public void extractWidgetRenderState(GuiGraphicsExtractor guiGraphics, int x, int y, float pt) {
        if (!visible) return;
        ...
        for (int i = 0; i < buttons.size(); i++) {
            buttons.get(i).extractWidgetRenderState(guiGraphics, x, y, pt);
            texts.get(i).extractWidgetRenderState(guiGraphics, x, y, pt);
        }
        if (currentId > 0)
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (visible && active)
            for (SelectButtonWidget<Identifier> button : buttons) {
