...
public class CommunicateSlotSelector extends AbstractWidget {
    ...
    @Override
    public void extractWidgetRenderState(@NotNull GuiGraphicsExtractor graphics, int _x, int _y, float p_268085_) {
        if (!visible) return;
        ...
        for (Pair<Vector2i, SlotType> slot : slots) {
            ...
            if (x >= pos.x && x < pos.x + SLOT.w && y >= pos.y && y < pos.y + SLOT.h) {
                ...
                graphics.setTooltipForNextFrame(Minecraft.getInstance().font, slot.getB().getName(), getX(), getY());
            }
        }
