...
public class VirtualFrameDataProvider implements IEntityComponentProvider {
    public static final Identifier VANILLA_ID = Identifier.tryParse("minecraft:item_frame");
    ...
    public void appendTooltip(ITooltip tooltip, EntityAccessor entityAccessor, IPluginConfig iPluginConfig) {
        ...
        if (itemStack.is(ItemRegistry.FILTER_LIST.get())) {
            ...
            for (ItemStack item : list) {
                if (item.isEmpty())
                    continue;
                Element tmpDisplay = JadeUI.item(item);
                tmpDisplay = tmpDisplay.offset(0, 0);
                tmpDisplay = tmpDisplay.size(18, 18);
    ...
    public Identifier getUid() {
        return Identifier.fromNamespaceAndPath(MaidStorageManager.MODID, "virtual_frame");
    }
}
