...
public class TaczRecipe {
    ...
    public static List<RecipeHolder<GunSmithTableRecipe>> getAllRecipesForBlockId(Level level, ResourceLocation blockId) {
        RecipeManager recipeManager = level.getRecipeManager();
        List<RecipeHolder<GunSmithTableRecipe>> allRecipesFor = recipeManager.getAllRecipesFor(ModRecipe.GUN_SMITH_TABLE_CRAFTING.get());
        ...
        if (commonBlockIndex.isPresent()) {
            ...
            allRecipesFor = allRecipesFor.stream().filter(recipe -> {
                ResourceLocation group = recipe.value().getResult().getGroup();
            ...
            if (!blockId.equals(DefaultAssets.DEFAULT_BLOCK_ID) || SyncConfig.ENABLE_TABLE_FILTER.get()) {
                ...
                if (filter != null)
                    allRecipesFor = filter.filter(allRecipesFor, RecipeHolder::id);
            }
        }
